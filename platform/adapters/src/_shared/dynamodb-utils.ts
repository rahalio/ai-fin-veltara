 ...
 *
 * @returns The core table name (e.g., "veltara-core-dev")
...
export function getCoreTableName(): string {
  const tableName = process.env.TABLE_NAME || process.env.DYNAMODB_CORE_TABLE_NAME || "veltara-core-local";
  if (!tableName) {
    throw new Error(
 ...
 *
 * @returns The base table name (e.g., "veltara-base-dev")
 ...
 *
 * @returns The analytics table name (e.g., "veltara-analytics-dev")
 ...
 *
 * @returns The realtime table name (e.g., "veltara-app-realtime-dev")
