/**
 * ContractRepositoryDdb — sandbox in-memory implementation (local/dev).
 * Hand-fit after Mode A codegen so product APIs work without DynamoDB Local.
 */

import type { ContractRepository } from "@veltara/services/marketplace";
...
export class ContractRepositoryDdb implements ContractRepository {
  private readonly store = sandboxEntityMap("marketplace:contract");

  constructor(_dynamoClient: AdapterDynamoDBClient) {}

  async listMarketplaceContracts(input: any): Promise<any> {
    const inputExt = asRecord(input);
    const items = [...this.store.values()];
    return { data: { items }, meta: meta(inputExt) } as any;
  }
  async createMarketplaceContract(input: any): Promise<any> {
    const inputExt = asRecord(input);
  ...
  async getMarketplaceContract(input: any): Promise<any> {
    const inputExt = asRecord(input);
    const id = entityId(inputExt, "id", "contractId");
